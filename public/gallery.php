  ...
  <div class="container">
    <div class="row g-4">
      ...
      <div class="col-lg-4 col-md-6 reveal">
        <div class="gallery-item" data-title="<?php echo htmlspecialchars($item['title']); ?>" style="position: relative; border-radius: var(--radius-md); overflow: hidden; aspect-ratio: 4/3; box-shadow: 0 10px 30px rgba(0,0,0,0.05); <?php echo pq_cover_style($item['image_url'] ?? '', 'var(--green)'); ?>">
          ...
          <div style="position: absolute; inset: 0; background: linear-gradient(to top, rgba(0,0,0,0.7), transparent); display: flex; align-items: flex-end; padding: 24px; opacity: 0; transition: var(--transition);" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0'">
            <div>
              <span style="color: var(--gold); font-size: 0.7rem; font-weight: 700; text-transform: uppercase;"><?php echo htmlspecialchars($item['cat'] ?? $item['category'] ?? ''); ?></span>
