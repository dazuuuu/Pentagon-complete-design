...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && Session::verifyCsrf($_POST['csrf'] ?? '')) {
  ...
  try {
    if ($action === 'save_settings') {
      ...
      Session::flash('message', 'Homepage settings saved.');
    } elseif ($action === 'upload_logo') {
      $service->uploadLogo($_FILES['logo'] ?? []);
      Session::flash('message', 'Logo uploaded. Transparency has been preserved.');
    } elseif ($action === 'upload_video') {
      $service->uploadHeroVideo($_FILES['hero_video'] ?? []);
      Session::flash('message', 'Homepage hero video uploaded.');
...
$heroVideoPath = $service->heroVideoPath();
$pageTitle = 'Homepage Media';
$currentAdminPage = 'homepage';
...
<div class="row g-3 mb-4">
  <div class="col-lg-4">
    <div class="content-card p-4 h-100">
      ...
      <div class="mb-3 p-3 rounded" style="background:#101010; min-height:110px; display:flex; align-items:center;">
        <img src="../<?php echo htmlspecialchars($logoPath); ?>" alt="Current logo" style="max-width:180px; max-height:80px; object-fit:contain;">
      </div>
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-text mb-3">Use a transparent PNG or WEBP for best results. The site will not recolor it.</div>
  ...
  <div class="col-lg-4">
    <div class="content-card p-4 h-100">
      <h5 class="mb-3">Homepage Hero Video</h5>
      <video src="../<?php echo htmlspecialchars($heroVideoPath); ?>" controls muted style="width:100%; height:150px; object-fit:cover; border-radius:12px; background:#111;"></video>
      <form method="POST" enctype="multipart/form-data" class="mt-3">
        <input type="hidden" name="csrf" value="<?php echo Session::csrfToken(); ?>">
        <input type="hidden" name="action" value="upload_video">
        <input class="form-control mb-2" type="file" name="hero_video" accept="video/mp4,video/webm,video/quicktime" required>
        <div class="form-text mb-3">MP4, WEBM, or MOV. Keep it short and compressed for fast loading.</div>
        <button class="btn btn-primary w-100">Upload Video</button>
      </form>
    </div>
  </div>
  ...
  <div class="col-lg-4">
    <div class="content-card p-4 h-100">
      <h5 class="mb-3">Poster Section</h5>
      <form method="POST">
        <input type="hidden" name="csrf" value="<?php echo Session::csrfToken(); ?>">
        <input type="hidden" name="action" value="save_settings">
        <div class="form-check form-switch mb-3">
          <input class="form-check-input" type="checkbox" role="switch" id="showPosters" name="show_posters" value="1" <?php echo $service->showPosters() ? 'checked' : ''; ?>>
...
<div class="content-card p-4 mb-4">
  ...
  <form method="POST" enctype="multipart/form-data" class="row g-2">
    ...
    <div class="col-md-3"><input class="form-control" name="description" placeholder="Short description"></div>
    <div class="col-md-2"><input class="form-control" name="link_url" placeholder="Link URL"></div>
    <div class="col-md-1"><input class="form-control" name="sort_order" type="number" placeholder="Order" value="0"></div>
    <div class="col-md-2">
...
<div class="content-card p-4">
  <h5 class="mb-3">Posters</h5>
  <table class="table table-sm align-middle">
    <thead><tr><th>Preview</th><th>Title</th><th>Description</th><th>Link</th><th>Order</th><th>Status</th><th></th></tr></thead>
    <tbody>
